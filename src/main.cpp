...
unsigned int nTargetSpacing = 1.5 * 60; //Block  spacing 2 minute
unsigned int nStakeMinAge = 24 * 60 * 60; //Minimum stake age 1 Day
unsigned int nStakeMaxAge = 24 * 60 * 60 * 7; //Maximum stake age 7 Day
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 77; //Coin Base Maturity
...
const string strMessageMagic = "Fuloos Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	int64_t nSubsidy = 1000 * COIN; //Miner reward
	...
	nSubsidy >>= (nBestHeight / 10000); // Halving: 10000 blocks 
...
static const int64_t nTargetTimespan = 7 * 60;  // 7 mins
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
	...
	if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
	{
		...
		printf("*** %s\n", strMessage.c_str());
		uiInterface.ThreadSafeMessageBox(strMessage, "Fuloos", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
		StartShutdown();
		return false;
	}
	return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	if (mapBlockIndex.empty())
	{
		...
		const char* pszTimestamp = "Fuloos Launch.";
		CTransaction txNew;
		txNew.nTime = 1512543600;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		...
		block.nVersion = 1;
		block.nTime = 1512543600;    //Wednesday, December 6, 2017 7:00:00 AM GMT
		block.nBits = bnProofOfWorkLimit.GetCompact();
		block.nNonce = !fTestNet ? 104781 : 104781;


		assert(block.hashMerkleRoot == uint256("0x989091008496febf39c9c46a1bfaf41aebfd485dbc9692f24b28bb2210f3f924"));
...
unsigned char pchMessageStart[4] = { 0x2a, 0xe7, 0x4c, 0x9d };
